            ...
            <div key={row}>
              <div className="flex min-h-[128px] items-end gap-2 px-4 opacity-30">
                {Array.from({ length: row === 0 ? 6 : 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="w-7 bg-[#4a3225] shadow-[2px_3px_6px_rgba(0,0,0,0.3)]"
                    style={{ height: 80 + (i % 3) * 16 }}
                  />
                ))}
              </div>
              <div className="h-2.5 rounded-sm bg-[#6B4A38]/60" />
            </div>
          ))}
        </div>

        <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
          <p className="font-display text-xl font-semibold text-white/90 sm:text-2xl">
            {hasActiveFilters
