...
import type { ReadingType, MonthlyReadingGoal } from "../../types/reading";
...
export default function MonthlyGoalCard() {
  const initialGoal = getMonthlyReadingGoal();

  const [goal, setGoal] = useState<MonthlyReadingGoal | null>(initialGoal);

  const [type, setType] = useState<ReadingType>(initialGoal?.type ?? "pages");

  const [target, setTarget] = useState(initialGoal?.target ?? 100);
  ...
  function handleSaveGoal() {
    ...
    const newGoal: MonthlyReadingGoal = {
      enabled: true,
      type,
      target,
    };

    setMonthlyReadingGoal(newGoal);
    setGoal(newGoal);
    setIsEditing(false);
  }
  ...
  function handleRemoveGoal() {
    ...
    const disabledGoal: MonthlyReadingGoal = {
      ...goal,
      enabled: false,
    };

    setMonthlyReadingGoal(disabledGoal);
  ...
  return (
    <section className="rounded-xl border border-white/10 bg-card-background p-6 shadow-lg">
      <header className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-semibold">Meta mensal</h2>

          <p className="mt-1 text-sm text-white/60">
            Defina quanto deseja ler mensalmente.
          </p>
        </div>
      </header>
      ...
      {goal?.enabled && (
        <div className="mt-6 rounded-lg border border-white/10 bg-black/20 p-5">
          ...
          <p className="text-white/70">
            {goal.type === "pages" ? "páginas por mês" : "minutos por mês"}
          </p>
        </div>
      )}
