...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    }).catch(cause => { if (!cancelled) setError(cause instanceof Error ? cause.message : "Falha ao carregar os dados."); });
  ...
  if (loading) return <p className="p-8 text-text">Carregando sua conta...</p>;
  ...
  if (userId && readyUser !== userId) return <main className="min-h-dvh bg-background p-8 text-text"><p role={error ? "alert" : "status"}>{error || "Carregando sua biblioteca na nuvem..."}</p>{error && <div className="mt-4 flex gap-3"><Button onClick={() => setAttempt(n => n + 1)}>Tentar novamente</Button><Button onClick={() => { void signOut(); }}>Sair</Button></div>}</main>;
